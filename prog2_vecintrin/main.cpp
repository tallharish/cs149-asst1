...
int main(int argc, char *argv[])
{
  ...
  printf("Doing clampedSerial\n");
  clampedExpSerial(values, exponents, gold, N);
  printf("Doing clampedVector\n");
...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  //
  __cs149_vec_float allOnes =  _cs149_vset_float(1.f);
  __cs149_vec_int allZerosI =  _cs149_vset_int(0);
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    ...
    _cs149_veq_int(maskYisZero, y, allZerosI, maskAll);  // y == 0
    _cs149_vset_float(result, oneF, maskYisZero);        // result[i] = 1.f
    maskYisNotZero = _cs149_mask_not(maskYisZero);      // else (y != 0)
